...
switchModel.findOne({ name: 'switch0' }, function(err, result) {
    if (err) {
...
switchModel.findOne({ name: 'switch1' }, function(err, result) {
    ...
    if (result == null){
        ...
        newSwitch.save(function(err) {
            if (err) throw err;
            console.log('Switch1 creado!');
        });
    };
});
//Populate si es la primera que corre.
switchModel.findOne({ name: 'switch2' }, function(err, result) {
    if (err) {
        return console.error(err);
    };
    if (result == null){
